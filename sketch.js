...
var obs1,obs2,obs3,obs4,out,obsgroup,foodgroup;
var start,oops,ops,strt,start1,foodImage,eat;

function preload(){
 
  eat=loadSound("eat.wav");
  foodImage=loadImage("Imported piskel (7).gif");
...
function setup() {
  ...
  strt=createSprite(width/2,height/2+10,20,20);
  strt.addImage(start);
...
function draw() {
  ...
  if(gamestate===start){
    ...
    text("3. Please rotate your mobile to landscape made for best experience",70,120);
    text("4. For every food you get +100 score",70,140);
    text("5. Click on the start button below to resume",70,160);
  ...
  if(gamestate===end){
    ...
    ops.visible=true;
    textSize(20);
    text("Tap anyware to restart",width/2-50,height/2);
    text("Press R to restart",width/2-50,height/2+30);
...
function fruits(){
  if(frameCount%240===0){
    var food=createSprite(width+5,20,20,20);
    food.y=Math.round(random(height-150,height-200));
    food.addImage(foodImage);
    food.scale=0.1;
    food.velocityX=-(6+score/80);
    foodgroup.add(food);
    foodgroup.setLifetimeEach(220);
  }
}
